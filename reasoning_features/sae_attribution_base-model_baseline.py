...
def compute_sae_attribution(ft_model: LanguageModel, base_model: LanguageModel, sae: SAE, tokens: torch.Tensor, attention_mask: torch.Tensor, metric_fn: Callable, sae_layer_idx: int) -> torch.Tensor:
    """
    ...
    with base_model.trace(tokens, kwargs={'attention_mask': attention_mask}):
        layer = base_model.model.layers[sae_layer_idx]
        hidden_state = layer.output[0]  # Shape: (batch, seq, d_model)
        hidden_state = hidden_state[~attention_mask]
        hidden_state = hidden_state.save().to(sae.device)
        base_sae_latents = ns.apply(
            sae.encode, hidden_state)  # Shape: (batch, seq, d_sae)
        base_sae_latents[~attention_mask] = 0
        base_sae_latents = base_sae_latents.save()

    baseline = base_sae_latents

    with ft_model.trace(tokens, kwargs={'attention_mask': attention_mask}):
        layer = ft_model.model.layers[sae_layer_idx]
        hidden_state = layer.output[0]  # Shape: (batch, seq, d_model)
        hidden_state = hidden_state[~attention_mask]
        hidden_state_grad = hidden_state.grad  # Shape: (batch, seq, d_model)

        # Ensure hidden state is on the correct device for SAE
        hidden_state = hidden_state.to(sae.device)
        sae_latents = ns.apply(
            sae.encode, hidden_state).save()  # Shape: (batch, seq, d_sae)
...
for i in range(0, num_sequences, BATCH_SIZE):
    ...
    batch_sae_attribution = compute_sae_attribution(
        base_model=base_model, ft_model=model, sae=sae, tokens=batch_tokens, attention_mask=batch_attention_mask,
        metric_fn=metric_fn_prepared, sae_layer_idx=SAE_LAYER
    )
