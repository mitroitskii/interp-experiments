...
def load_tokenizer():
    return AutoTokenizer.from_pretrained("deepseek-ai/DeepSeek-R1-Distill-Llama-8B")
...
def analyze_token_stats(tokenizer, dataset, field='message_llama_chat_template'):
...
def main():
    ...
    dataset = load_dataset("koyena/Magpie-Reasoning-V2-250K-CoT-Deepseek-R1-Llama-70B-formatted")['train']

    print("Computing token statistics...")
    stats, token_counts = analyze_token_stats(tokenizer, dataset)

    print("\nToken count statistics for 'message_llama_chat_template' field:")
