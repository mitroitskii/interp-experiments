...
source_dataset = "Magpie-Align/Magpie-Reasoning-V2-250K-CoT-Deepseek-R1-Llama-70B"
target_dataset = "koyena/Magpie-Reasoning-V2-250K-CoT-Deepseek-R1-Llama-70B-formatted"
# %%
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset-config", type=str, default="train") # if the dataset has multiple configs
...
# %%
    ...
    messages = [
        ...
        for instruction, response in zip(ds["instruction"], ds["response"])
    ]
    ds = ds.add_column("message", messages)
# %%
    # split and shuffle
    splits = ds.train_test_split(test_size=0.3, seed=42)
    splits["validation"] = splits.pop("test")
    ds = splits
