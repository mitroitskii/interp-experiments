...
def load_models() -> Tuple[
    ...
    ft_model = AutoModelForCausalLM.from_pretrained(
        FT_MODEL_NAME, device_map="auto", torch_dtype=th.bfloat16)
    ...
    base_model = AutoModelForCausalLM.from_pretrained(
        BASE_MODEL_NAME, device_map="auto", torch_dtype=th.bfloat16)
...
def load_dataset_hf(
    dataset_path: str = "mitroitskii/OpenR1-Math-220k-formatted",
...
def analyze_generation(
    ...
    for data in tqdm(dataset, desc="Generating Samples"):
        ...
        if isinstance(data.get('problem'), list) and data['problem']:
             prompt_content = data['problem'][0]['content']
        elif isinstance(data.get('problem'), str):
             prompt_content = data['problem']
        else:
